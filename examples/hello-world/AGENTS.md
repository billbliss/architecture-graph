...
Before changing that responsibility or the formatting rules, run `npm run ag -- context --file src/formatter.js` (or use the affected file) and read the returned design. Update `architecture/graph.json` alongside the implementation when the design changes.
