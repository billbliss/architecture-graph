...
function run(cwd,bin,args,status=0) {
  const result=spawnSync(bin,args,{cwd,encoding:'utf8',env:{...process.env,npm_config_cache:join(temporary,'npm-cache')}});
...
function save(cwd,graph) {writeFileSync(join(cwd,'architecture/graph.json'),JSON.stringify(graph,null,2)+'\n');}
try {
  let tarball = process.env.AG_TARBALL && resolve(process.env.AG_TARBALL);
  if (!tarball) {
    const packed=JSON.parse(execFileSync('npm',['pack','--json','--pack-destination',temporary,'--ignore-scripts'],{cwd:repo,encoding:'utf8',env:{...process.env,npm_config_cache:join(temporary,'npm-cache')}}))[0];
  ...
  const final=JSON.parse(readFileSync(join(example,'architecture/graph.json')));
