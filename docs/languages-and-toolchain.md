...
The toolkit is implemented in TypeScript, but using it does not require TypeScript knowledge. Its references name local files regardless of language. AG does not parse your application's code or depend on a particular language's type system. The intended workflow has a coding agent interpret the code and maintain the declarations under human review; the toolkit checks the records and retrieves relevant context.
...
A coding agent is a practical requirement for sustained use. The manual CLI remains available, but treating every contributor as responsible for remembering a separate graph-maintenance process is not the intended workflow.
...
| Repository files | Add a `package.json` and npm lockfile if absent, plus AG's configuration and graph. If installing from a tarball, keep the installed archive in `vendor/` so teammates and CI can reproduce the installation. |
...
The tarball contains AG, but npm also installs its YAML and schema dependencies. A first installation needs registry access unless those dependencies are already cached. Installing by name from npm simplifies acquiring and updating AG; it does not remove the Node requirement. There is currently no standalone executable or native Python, Rust or .NET distribution.
